...
class Investigate:
    """Runs the three classifier investigations from Session 8."""

    def __init__(self) -> None:
        """Initialise the investigation harness.
    ...
    def balvsunbal(self) -> None:
        """Investigation 1: compare balanced vs unbalanced CatBoost on weeks 1+2 → 3.

        Trains both class-weight variants on the same data and prints precision,
        recall, and flag-rate at each probability threshold.
    ...
    def increase_training_data(self) -> None:
        """Investigation 2: sweep training size (500k vs 2M) across both splits.

        Determines whether additional training data improves classifier
        performance. Prints AP and Brier score for each combination.
    ...
    def asset_agnostic(self) -> None:
        """Investigation 3: compare models with and without the asset_id feature.

        Tests whether an explicit asset indicator improves performance over
        microstructure features alone. Prints metrics for each combination.
